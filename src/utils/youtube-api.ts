...
export const fetchVideoAnalytics = async (
  ...
  console.log(`\n[API] Fetching analytics for video: ${videoId}`);
  ...
  const countryCodeMap: { [key: string]: string } = {
    ...
    'MX': 'Mexico', 'JP': 'Japan', 'PH': 'Philippines', 'EG': 'Egypt',
    'VN': 'Vietnam', 'TR': 'Turkey', 'DE': 'Germany', 'TH': 'Thailand',
    'GB': 'United Kingdom', 'FR': 'France', 'IT': 'Italy', 'ZA': 'South Africa',
    'KR': 'South Korea', 'ES': 'Spain', 'AR': 'Argentina', 'CA': 'Canada',
    'MY': 'Malaysia', 'PL': 'Poland', 'AU': 'Australia', 'SA': 'Saudi Arabia',
    'PE': 'Peru', 'NL': 'Netherlands', 'IQ': 'Iraq', 'NP': 'Nepal',
    'JO': 'Jordan', 'LB': 'Lebanon',
  };

  try {
    const [demographicsData, geographyData, deviceData, watchTimeData] = await Promise.all([
      fetch(
        `${YOUTUBE_ANALYTICS_API}/reports?ids=channel==MINE&startDate=${startDate}&endDate=${today}&dimensions=ageGroup,gender&metrics=viewerPercentage&filters=video==${videoId}`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      ).then(res => res.json()),

      fetch(
        `${YOUTUBE_ANALYTICS_API}/reports?ids=channel==MINE&startDate=${startDate}&endDate=${today}&dimensions=country&metrics=views,estimatedMinutesWatched&filters=video==${videoId}&sort=-views`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      ).then(res => res.json()),

      fetch(
        `${YOUTUBE_ANALYTICS_API}/reports?ids=channel==MINE&startDate=${startDate}&endDate=${today}&dimensions=deviceType&metrics=views&filters=video==${videoId}`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      ).then(res => res.json()),

      fetch(
        `${YOUTUBE_ANALYTICS_API}/reports?ids=channel==MINE&startDate=${startDate}&endDate=${today}&metrics=views,likes,comments,shares,estimatedMinutesWatched,averageViewDuration,averageViewPercentage&filters=video==${videoId}`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      ).then(res => res.json()),
    ]);
    ...
    console.log('[API] Video watch time data:', watchTimeData);
    ...
    (demographicsData.rows || []).forEach((row: any[]) => {
      ...
      ageGroupMap.set(ageGroup, (ageGroupMap.get(ageGroup) || 0) + percentage);
      genderMap.set(gender, (genderMap.get(gender) || 0) + percentage);
    ...
    const totalViews = deviceData.rows ? deviceData.rows.reduce((sum: number, row: any[]) => sum + row[1], 0) : 0;

    const watchTimeRow = watchTimeData.rows?.[0] || [];

    const analyticsData: VideoAnalytics = {
      videoId,
      demographics: {
        age: Array.from(ageGroupMap.entries()).map(([ageGroup, percentage]) => ({
          ageGroup,
          percentage: Math.round(percentage * 100) / 100,
        })),
        gender: Array.from(genderMap.entries()).map(([gender, percentage]) => ({
        ...
        ageGenderBreakdown: Array.from(ageGenderMap.entries()).map(([ageGroup, data]) => ({
          ageGroup,
          male: Math.round(data.male * 100) / 100,
          female: Math.round(data.female * 100) / 100,
          total: Math.round((data.male + data.female) * 100) / 100,
        })),
      },
      geography: (geographyData.rows || []).map((row: any[]) => {
        const countryCode = row[0];
        const views = row[1];
        const estimatedMinutesWatched = row[2] || 0;
        const geoTotalViews = (geographyData.rows || []).reduce((sum: number, r: any[]) => sum + r[1], 0);
        const percentage = geoTotalViews > 0 ? (views / geoTotalViews) * 100 : 0;
        const countryName = countryCodeMap[countryCode] || countryCode;
        return {
          country: countryName,
          percentage: Math.round(percentage * 100) / 100,
      ...
      watchTime: {
        estimatedMinutesWatched: watchTimeRow[4] || 0,
        averageViewDuration: watchTimeRow[5] || 0,
        averageViewPercentage: watchTimeRow[6] || 0,
      },
      engagement: {
        views: watchTimeRow[0] || 0,
        likes: watchTimeRow[1] || 0,
        comments: watchTimeRow[2] || 0,
        shares: watchTimeRow[3] || 0,
      },
    };
